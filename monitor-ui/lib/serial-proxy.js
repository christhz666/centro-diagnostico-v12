...
class SerialProxy {
    ...
    async iniciar() {
        ...
        try {
            // ── Cargar serialport saltando el snapshot de pkg ─────────
            // Cuando corre como .exe (process.pkg = true), pkg embebe el
            // código JS en un snapshot virtual (C:\snapshot\...) pero los
            // binarios nativos .node DEBEN cargarse desde el disco real.
            // createRequire con la carpeta real del exe soluciona esto.
            let SerialPort;
            if (process.pkg) {
                // Cargar desde la carpeta real junto al exe (no del snapshot)
                const { createRequire } = require('module');
                const requireDesdeExe = createRequire(
                    require('path').join(require('path').dirname(process.execPath), 'node_modules', 'serialport', 'package.json')
                );
                SerialPort = requireDesdeExe('serialport').SerialPort;
            } else {
                // Modo normal (node server.js)
